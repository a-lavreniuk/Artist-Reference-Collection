...
description: >-
  ...
  карточки в Buildin. Use when the user says «заверши задачу», «закрой задачу»,
  «смержи и удали ветку», or asks to push code to GitHub and merge into main.
---

# Завершить задачу (Git + Buildin)

Закрывает цикл разработки: сохранить код на GitHub, влить тестовую ветку в `main`, удалить ветку, **затем** обновить или создать карточку в Buildin (доска **Таски**).
...
При падении — **остановиться**, сообщить пользователю. Merge, push и Buildin **не делать**.
...
- план: push → merge в main → удалить ветку → Buildin.
...
- При **конфликте merge** — **остановиться**, не решать самому, сообщить пользователю. Buildin не трогать.
...
По skill **buildin-task-finish** (`.cursor/skills/buildin-task-finish/`):

1. Выполнить **после** успешного git-финала (или сразу, если git не требовался).
2. Обновить карточку по `buildin_page_id` из сессии (**buildin-task-take**) или создать новую для задачи из чата.
3. Статус → **«Готово»**; в теле — секция **«Что сделано»** (блоки `heading_1` + `divider`, см. task-card-format).
4. Другие свойства карточки не менять.

Если пользователь закрывает задачу с **незавершённой** работой — всё равно «Готово», в «Что сделано» указать, что не доделано.
...
- Не обновлять Buildin при остановке на тестах, конфликте или critical review (кроме явной просьбы пользователя закрыть как есть).

## 8. Связка с другими skills

- Старт задачи и `page_id` — **buildin-task-take** («возьми задачу»).
- Ревью перед merge — **arc-code-review** (шаг 2b).
- UI-ревью внутри code review — **arc-ui-dev** (review-template).
- Закрытие карточки Buildin — **buildin-task-finish** (шаг 6).
...
6. **buildin-task-finish**: статус «Готово», секция «Что сделано» (divider-блоки)  
7. Отчёт: хеш merge-коммита, ссылка на карточку Buildin
