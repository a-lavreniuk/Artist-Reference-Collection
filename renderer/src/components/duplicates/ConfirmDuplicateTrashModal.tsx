...
export default function ConfirmDuplicateTrashModal({ libraryName, onClose, onConfirm }: Props) {
  ...
  return (
    <ArcAnimatedModalHost onClose={onClose}>
      {({ requestClose }) => (
        <FloatingModalPanel
          ref={hostRef}
          panelId="confirm-duplicate-trash-modal"
          className="arc-modal"
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="arcDuplicateTrashTitle"
          onClick={(e) => e.stopPropagation()}
        >
          <header className="arc-modal__header arc-modal__header--title">
            <h3 className="arc-modal__title" id="arcDuplicateTrashTitle">
              Удалить копию?
            </h3>
          ...
          <div className="arc-modal__body">
            <div className="arc-modal__slot">
              <p className="arc-modal__slot-text">
                Карточка переместится в корзину библиотеки «{libraryName}». Её можно будет восстановить позже.
              </p>
            </div>
          </div>
          <footer className="arc-modal__footer arc-modal__footer--actions-3">
            ...
            >
              <span className="btn-ds__value">{busy ? 'Удаление…' : 'В корзину'}</span>
            </button>
