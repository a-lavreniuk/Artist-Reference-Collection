...
type Props = {
  /** Ключ геометрии плавающей панели: своя позиция/размер на каждый сценарий. */
  panelId: string;
  ariaLabel: string;
  onClose: () => void;
  renderTagChip: (tag: TagRecord, category: CategoryRecord) => ReactNode;
  onTagCreated: (tagId: string) => void | Promise<void>;
  /** Строка над поиском: контекст выборки (например, число выбранных карточек). */
  contextNote?: ReactNode;
  /** Меняется при внешнем обновлении состояния чипов — повторная гидрация иконок. */
  hydrateKey?: unknown;
};
...
/**
 * Плавающий пикер меток: сайдбар категорий, поиск, группы чипов и вложенные
 * модалки создания метки / категории. Вид чипа задаёт вызывающий экран.
 */
        ...
        >
          <div className="arc-add-tags-picker__content-fixed">
            <div className="arc-add-tags-picker__content-inset">
              {contextNote ? (
                <p className="text-s arc-add-tags-picker__context-note">{contextNote}</p>
              ) : null}
              <div
