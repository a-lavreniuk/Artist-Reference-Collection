...
import { canPlayInBrowser } from '../../utils/media-formats';
  ...
  const handleOpenInSystemPlayer = async () => {
    if (window.electronAPI) {
      try {
        ...
        await window.electronAPI.openExternal(`file:///${card.filePath.replace(/\\/g, '/')}`);
      } catch (error) {
        console.error('Ошибка открытия в системном плеере:', error);
        alert.error('Не удалось открыть файл в системном плеере');
      }
    }
  };
      ...
      >
        ...
        <div className="card-view__content">
          ...
          {card.type === 'image' ? (
            ...
            <>
              {(() => {
                // Проверяем может ли формат воспроизводиться в браузере
                const videoFormat = card.format as VideoFormat;
                const isBrowserCompatible = canPlayInBrowser(videoFormat);

                if (!isBrowserCompatible) {
                  // Для несовместимых форматов показываем превью и кнопку открытия
                  return (
                    <div className="card-view__unsupported-video">
                      <img
                        src={card.thumbnailUrlStandard || card.thumbnailUrl || ''}
                        alt={card.fileName}
                        className="card-view__image"
                        style={{ filter: 'brightness(0.7)' }}
                      />
                      <div className="card-view__video-overlay">
                        <p className="card-view__video-message">
                          Формат {card.format.toUpperCase()} не поддерживается для воспроизведения в приложении
                        </p>
                        <Button
                          variant="primary"
                          size="L"
                          onClick={handleOpenInSystemPlayer}
                          iconLeft={<Icon name="play" size={24} variant="border" />}
                        >
                ...
                // Для совместимых форматов показываем обычное видео
                if (isLoadingVideo) {
                  return (
