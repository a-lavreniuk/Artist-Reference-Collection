...
export const TagsPage = () => {
  ...
  const loadData = async (preserveScroll = false) => {
    try {
      ...
      // Проверяем наличие дубликатов order
      const orderValues = allCategories.map(c => c.order).filter(o => o !== undefined);
      const hasDuplicates = orderValues.length !== new Set(orderValues).size;
      
      if (hasDuplicates) {
        console.warn('[TagsPage] Обнаружены дубликаты order, исправляем...');
        const { fixCategoriesOrder } = await import('../services/db');
        await fixCategoriesOrder();
        // Перезагружаем категории после исправления
        const fixedCategories = await getAllCategories();
