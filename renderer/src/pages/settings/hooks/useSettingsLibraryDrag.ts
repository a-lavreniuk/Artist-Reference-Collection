...
export type SettingsLibraryDragState = {
  dragId: string;
  insertIndex: number;
  ghostX: number;
  ghostY: number;
...
function collectRowMetrics(listEl: HTMLElement): RowMetrics[] {
  const nodes = listEl.querySelectorAll<HTMLElement>('[data-settings-library-row]');
  return Array.from(nodes).map((node) => ({
    id: node.dataset.settingsLibraryRow ?? '',
...
export function useSettingsLibraryDrag(onReorder: (id: string, insertIndex: number) => void) {
  const [dragState, setDragState] = useState<SettingsLibraryDragState | null>(null);
