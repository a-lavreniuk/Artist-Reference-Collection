...
export const changelog: VersionChange[] = [
  {
    ...
    date: '31.12.2025',
    changes: [
      'Исправлена критическая ошибка с подсчетом карточек — теперь счетчики показывают реальное количество файлов в системе сразу при запуске',
      'Исправлена фильтрация по типу контента — при клике на "Видео" или "Изображения" карточки загружаются из базы данных',
      'Исправлен поиск по меткам — теперь работает по всей базе данных, а не только по подгруженным карточкам',
      'Оптимизирован поиск карточек через использование индексов IndexedDB для максимальной производительности',
