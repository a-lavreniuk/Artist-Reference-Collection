...
import { ARC_INTERFACE_TOUR_REPLAY_EVENT } from '../components/onboarding/interfaceTourEvents';
...
export function useInterfaceTour() {
  ...
  useEffect(() => {
    ...
    if (prefs.onboardingSetupCompleted && !prefs.onboardingTourCompleted) {
      autoStartedRef.current = true;
      setStepIndex(prefs.onboardingTourStep);
      setActive(true);
    }
  }, [prefs, ready]);
