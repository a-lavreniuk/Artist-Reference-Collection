 ...
 * @returns {string | null}
 */
export function extractArtworkHashFromUrl(url) {
  if (!url?.trim()) return null;
  return ARTWORK_PATH_RE.exec(url.split('?')[0])?.[1] ?? null;
}
 ...
 */
export function buildArtstationProjectCardName(title, author, index = 0, total = 1) {
  const base =
 ...
 */
export function parseArtstationProjectJson(data, artworkUrl) {
  if (!data || typeof data !== 'object' || !artworkUrl?.trim()) return null;

  const artworkId = extractArtworkHashFromUrl(artworkUrl);
  ...
  for (const asset of assets) {
    ...
    if (entry.has_image && typeof entry.image_url === 'string' && entry.image_url.trim()) {
      const url = stripArtstationResizeSegment(entry.image_url.replace(/\\\//g, '/'));
      images.push({
        url,
    ...
    if (!videoUrl && entry.has_embedded_player && typeof entry.player_embedded === 'string') {
      const embedded = entry.player_embedded;
      const srcMatch = /src=["']([^"']+)["']/i.exec(embedded);
