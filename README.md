...
2. Запустите **ARC Setup 1.2.8.exe**
...
└── _cache/
    ...
    └── imports/          # Временные файлы из браузера (автоочистка через 24ч)
```
...
- **Drag-and-Drop для меток**
  - Добавлена возможность перемещать метки между категориями с помощью drag-and-drop
  - Визуальная обратная связь — зеленая обводка категории при наведении перетаскиваемой метки
  - Привязка меток к карточкам сохраняется при перемещении между категориями
  - Автоматическая синхронизация массивов `tagIds` в категориях
- **Опциональные метки**
  - Убрано ограничение на обязательные метки при добавлении карточек
  - Карточки можно добавлять в систему без меток
