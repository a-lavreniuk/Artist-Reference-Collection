...
Токен уже входит в пакет **Копировать** (`Authorization: Bearer …`). Он отдельный от секрета расширения браузера. Менять секрет нужно редко: кнопка «Сгенерировать новый секрет», затем снова «Копировать» и обновить конфиг клиента.
...
```json
{
  "mcpServers": {
    "arc-mcp": {
      "url": "http://127.0.0.1:47897/mcp",
      "headers": {
        "Authorization": "Bearer <токен из пакета Копировать>"
      }
    }
  }
}
