...
- Есть перенос папки и relink. Пользовательского ZIP-бэкапа (`.arc`) в продукте нет (runtime `backupLibrary` / `restoreLibrary` удалён).
...
| 2 | **API upload бэкапов** — OAuth + выгрузка снимка библиотеки на Яндекс Диск (потом опционально другие провайдеры) | M | Высокая; потребует заново собрать упаковку библиотеки |
...
Согласованный снимок библиотеки должен закрывать SQLite (`closeLibraryDb()`) **до** копирования файлов.

### 2.3 ZIP-бэкап (снят с продукта)

Ранее в репозитории были `backupLibrary.ts` / `restoreLibrary.ts` (формат `.arc` / `.arc.partNN`). UI панели не было; в августе 2026 runtime удалён как неиспользуемый. Для облачного сценария упаковку нужно проектировать заново.
...
2. **Фаза 1 — следующая задача на код**  
   - После сборки снимка библиотеки (или выбор существующего архива) — upload в папку приложения на Яндекс Диске.  
   - Список бэкапов в облаке, скачивание → restore.  
